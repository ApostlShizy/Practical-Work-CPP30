...
Реализация : Пользователь вводи ключ после программа предлагает ввести агрумент для данного ключа.Затем ключ и аргумент помещаются в std::map. Цикл повторяется пока в качестве "ключа" не будет введено "post" или "get". На этом цикл заканчивается. После в зависимости от выбранной опции сереверу передатёся или cpr::Post запрос с телом в включающем в себя все ключи и аргументы или cpr::Get где аргументы помещаются прямо в url ссылку. После выполнение программы заканчивается. В случай если ни одного ключа и аргумента не было сформировано программы сообщит об этом и завершится.
